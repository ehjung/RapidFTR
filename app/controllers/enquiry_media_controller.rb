class EnquiryMediaController < ApplicationController
  helper :enquiries 
  ...
  def find_enquiry
    @enquiry = Enquiry.get(params[:enquiry_id])
  end

  def find_audio_attachment
    begin
      @attachment = params[:id] ? @enquiry.media_for_key(params[:id]) : @enquiry.audio
    rescue => e
      p e.inspect
    end
  end
